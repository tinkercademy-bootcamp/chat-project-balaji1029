...
void check_error_and_close(bool test, const std::string &error_message, int sock) {
  if (test) {
    std::cerr << error_message << "\n";
    close(sock);
    exit(EXIT_FAILURE);
...
void bind_address_to_socket(int sock, sockaddr_in &address) {
  ...
  // }
  check_error_and_close(
      bind(sock, (sockaddr *)&address, sizeof(address)) < 0, "bind failed", sock);
}
...
void listen_on_socket(int sock) {
  ...
  //   exit(EXIT_FAILURE);
  // }
  check_error_and_close(listen(sock, 3) < 0, "listen failed", sock);
}
